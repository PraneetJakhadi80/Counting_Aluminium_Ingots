...
# Define the function to read camera credentials from the database
def read_camera_credentials():
    ...
    try:
        ...
        # Fetch the camera credentials from the database
        cursor.execute("SELECT ip_address, username, password, rtsp_port, camera_id FROM camera_credentials")
        row = cursor.fetchone()

        # Check if the row is not empty
        if row:
            ip_address, username, password, rtsp_port, camera_id = row
            return {
...
camera_credentials = read_camera_credentials()
...
if camera_credentials:
    ...
    camera_id = camera_credentials.get('camera_id')

    # Check if the necessary credentials are present
    if not all([ip_address, username, password, rtsp_port, camera_id]):
        print("Incomplete camera credentials. Please ensure all fields are populated.")
        exit()
